...
) 
{
    ...
    for (int i = 0; i < spp; i++) {
        ...
        while(nBounces--) {
            ...
            evalBRDF(ray.info.norm, -ray.direction, mat, &state, newdir, weight);
            attenuation *= weight;
            ray.set_origin_and_direction(ray.origin + ray.direction * ray.info.t + ray.info.norm * 0.001f, newdir);
